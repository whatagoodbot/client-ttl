...
export const init = async (sessionID) => {
  session = {
    ...
    headers: {
      ...
      Cookie: process.env.BARD_COOKIE_FULL
    }
  }
...
export const queryBard = async (message, ids = {}) => {
  if (!SNlM0e) { throw new Error('Make sure to call Bard.init(SESSION_ID) first.') }
  ...
  const text = bardResponseData[1][0]

  const images = bardResponseData[4]?.map((x) => {
    return {
      tag: x[2],
      url: x[3][0][0],
      source: {
